...
- (void)addNetworkingImageToAlbum:(NSArray<NSString *> *)imageUrls selected:(BOOL)selected {
    ...
    for (NSString *imageUrlStr in imageUrls) {
        ...
        if ([imageUrlStr isKindOfClass:[NSString class]]) {
            NSString *hz = [imageUrlStr componentsSeparatedByString:@"."].lastObject;
            if ([hz isEqualToString:@"mp4"] || [hz isEqualToString:@"mov"]) {
                NSURL *theURL = photoModel.networkPhotoUrl;
                photoModel.type = HXPhotoModelMediaTypeCameraVideo;//HXPhotoModelMediaTypeVideo;
                photoModel.subType = HXPhotoModelMediaSubTypeVideo;
                photoModel.cameraVideoType = HXPhotoModelMediaTypeCameraVideoTypeNetWork;
                photoModel.videoURL = theURL;
                photoModel.networkPhotoUrl = nil;
                if (theURL) {
                    NSString *theFilePath = theURL.absoluteString;
                    if (![theFilePath containsString:@"x-oss-process"]) {
                        ...
                        theFilePath = [NSString stringWithFormat:@"%@?x-oss-process=video/snapshot,t_10000,m_fast",theFilePath];
                    }
//                    /**  临时的预览大图  - 本地图片才用这个上传 */
//                    dispatch_async(dispatch_get_global_queue(0, 0), ^{
                        photoModel.previewPhoto = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:theFilePath]]];;
//                    });
                }
            }
        }
...
//    [self.iCloudUploadArray removeAllObjects];
    [self clearCancelSelectList];
}

- (void)clearCancelENDList {
    [self.endSelectedList removeAllObjects];
    [self.endCameraPhotos removeAllObjects];
    [self.endSelectedCameraPhotos removeAllObjects];
    [self.endCameraList removeAllObjects];
...
- (void)clearCancelENDListWithList:(NSArray *)modelList {
    NSMutableArray *theMArr = [NSMutableArray new];
    for (HXPhotoModel *theModel in modelList) {
        if (theModel.networkPhotoUrl || theModel.videoURL) {
            [theMArr addObject:theModel];
        }
    }
    [self removeModelInArray:self.endSelectedList blArr:theMArr];
    [self removeModelInArray:self.endCameraPhotos blArr:theMArr];
    [self removeModelInArray:self.endSelectedCameraPhotos blArr:theMArr];
    [self removeModelInArray:self.endCameraList blArr:theMArr];
    [self removeModelInArray:self.endSelectedCameraList blArr:theMArr];
    [self removeModelInArray:self.endSelectedPhotos blArr:theMArr];
    [self removeModelInArray:self.endCameraVideos blArr:theMArr];
    [self removeModelInArray:self.endSelectedCameraVideos blArr:theMArr];
    [self removeModelInArray:self.endSelectedVideos blArr:theMArr];
    
    self.endIsOriginal = NO;
    self.endPhotosTotalBtyes = nil;
}

- (void)removeModelInArray:(NSMutableArray *)array blArr:(NSArray *)blArr {
    for (NSInteger i = array.count-1 ; i >= 0 ; i --) {
        HXPhotoModel *nModel = [array objectAtIndex:i];
        BOOL result = YES;
        for (HXPhotoModel *bModel in blArr) {
            if ([bModel isEqual:nModel] || [bModel.asset isEqual:nModel.asset]) {
                result = NO;
                break;
            }
        }
        if (result) {
            [array removeObject:nModel];
        }
    }
    // 更新选择索引
    for (int i = 0 ; i < array.count ; i ++) {
        HXPhotoModel *yModel = [array objectAtIndex:i];
        yModel.selectedIndex = i; // i
        yModel.selectIndexStr = [NSString stringWithFormat:@"%d",i + 1]; // i+1
    }
}

- (void)clearCancelSelectList {
    [self.selectedList removeAllObjects];
    [self.cameraPhotos removeAllObjects];
    [self.selectedCameraPhotos removeAllObjects];
    [self.cameraList removeAllObjects];
