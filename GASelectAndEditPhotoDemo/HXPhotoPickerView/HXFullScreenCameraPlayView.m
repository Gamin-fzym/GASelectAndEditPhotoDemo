...
// 拍照按钮
@property (nonatomic, strong) UIImageView *pzPlayImageView;
...
- (void)setupPhotoPlayWithStatus:(BOOL)status {
    if (status) {
        self.whiteCircleLayer.hidden = YES;
        self.progressLayer.hidden = YES;
        self.circleLayer.hidden = YES;
        self.pzPlayImageView.frame = CGRectMake(-2, -2, self.frame.size.width+4, self.frame.size.height+4);
        [self addSubview:self.pzPlayImageView];
    } else {
        self.whiteCircleLayer.hidden = NO;
        self.progressLayer.hidden = NO;
        self.circleLayer.hidden = NO;
        [self.pzPlayImageView removeFromSuperview];
    }
}

- (UIImageView *)pzPlayImageView {
    if (!_pzPlayImageView) {
        _pzPlayImageView = [[UIImageView alloc] init];
        _pzPlayImageView.backgroundColor = [UIColor clearColor];
        _pzPlayImageView.userInteractionEnabled = NO;
        _pzPlayImageView.image = [UIImage hx_imageNamed:@"pz_play"];
    }
    return _pzPlayImageView;
}
