...
@property (strong, nonatomic) UIImageView *videoMarkIV;
@end
...
- (UIImageView *)videoMarkIV {
    if (!_videoMarkIV) {
        _videoMarkIV = [[UIImageView alloc] init];
        _videoMarkIV.backgroundColor = [UIColor clearColor];
        _videoMarkIV.userInteractionEnabled = NO;
        _videoMarkIV.image = [UIImage imageNamed:@"play_normal.png"];
    }
    return _videoMarkIV;
}
...
- (void)setup {
    ...
    [self.contentView addSubview:self.videoMarkIV];
...
- (void)setModel:(HXPhotoModel *)model {
    ...
    if (model.type == HXPhotoModelMediaTypePhotoGif) {
        ...
        if (model.subType == HXPhotoModelMediaSubTypeVideo) {
            ...
            if (![model.videoTime isKindOfClass:[NSNull class]] && ([model.videoTime isKindOfClass:[NSString class]] && ![model.videoTime isEqualToString:@"00:00"])) {
                self.stateLb.hidden = NO;
            } else {
                self.stateLb.hidden = YES;
                self.videoMarkIV.hidden = NO;
            }
            self.bottomMaskLayer.hidden = NO;
        }else {
...
- (void)layoutSubviews
{
    ...
    float bf_w = 40.0;
    self.videoMarkIV.frame = CGRectMake((self.hx_w-bf_w)/2.0, (self.hx_h-bf_w)/2.0, bf_w, bf_w);
