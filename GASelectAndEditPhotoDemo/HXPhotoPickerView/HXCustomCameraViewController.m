...
- (void)setupInterceptionView {
    ...
    _interceptionView.center = CGPointMake(_previewView.frame.size.width/2.0, _previewView.frame.size.height/2.0);
    [self.previewView addSubview:_interceptionView];
...
- (void)takePicturesComplete:(UIImage *)image {
    ...
    // 拍照后会弹出确认页
    self.imageView.image = [GAPublicClass cropImageWithOldImage:image OldFrame:CGRectMake(0, 0, self.view.hx_w, self.view.hx_h) CropFrame:_interceptionView.frame];
    [self.view insertSubview:self.imageView belowSubview:self.bottomView];
    [self.view addSubview:self.doneBtn];
    [self.cameraController stopSession];
    self.cancelBtn.hidden = NO;
...
- (void)playViewInitMode:(HXCustomCameraBottomViewMode)mode {
...
- (void)playViewChangeMode:(HXCustomCameraBottomViewMode)mode {
    if (mode == HXCustomCameraBottomViewModePhoto) {
        ...
        self.interceptionView.hidden = NO;
        [self.bottomView.playView setupPhotoPlayWithStatus:YES];
    }else {
        [self requestAccessForAudio];
        [self.cameraController addMovieOutput];
        self.interceptionView.hidden = YES;
        [self.bottomView.playView setupPhotoPlayWithStatus:NO];
    }
    [self setupFlashAndTorchBtn];
}
...
- (HXCustomPreviewView *)previewView {
    if (!_previewView) {
        ...
        if ([UIDevice currentDevice].userInterfaceIdiom == UIUserInterfaceIdiomPad) {
            ...
            _previewView.hx_size = CGSizeMake(self.view.hx_w, self.view.hx_h);
            _previewView.center = CGPointMake(self.view.hx_w / 2, self.view.hx_h / 2);
        }
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        _cancelBtn.hx_x = 16;
        _cancelBtn.hx_y = [UIScreen mainScreen].bounds.size.height - _cancelBtn.hx_size.height - 26;
    }
    return _cancelBtn;
}
- (UIButton *)changeCameraBtn {
    if (!_changeCameraBtn) {
        ...
        _changeCameraBtn.hx_x = [UIScreen mainScreen].bounds.size.width - _changeCameraBtn.hx_size.width - 16;
        _changeCameraBtn.hx_y = [UIScreen mainScreen].bounds.size.height - _changeCameraBtn.hx_size.height - 26;
    }
...
- (UIButton *)flashBtn {
    if (!_flashBtn) {
        ...
        _flashBtn.hx_size = normalImage.size;
        _flashBtn.hx_x = [UIScreen mainScreen].bounds.size.width - _flashBtn.hx_size.width - 20;
        _flashBtn.hx_y = 16;
    }
    return _flashBtn;
}
- (HXCustomCameraBottomView *)bottomView {
    if (!_bottomView) {
        _bottomView = [[HXCustomCameraBottomView alloc] initWithFrame:CGRectZero manager:self.manager isOutside:self.isOutside delegate:self];
...
- (UIButton *)doneBtn {
    if (!_doneBtn) {
        ...
        _doneBtn.hx_size = CGSizeMake(50, 50);
        _doneBtn.hx_x = [UIScreen mainScreen].bounds.size.width - _doneBtn.hx_size.width - 16;
        _doneBtn.hx_y = [UIScreen mainScreen].bounds.size.height - _doneBtn.hx_size.height - 26;
    }
    return _doneBtn;
}
...
- (instancetype)initWithFrame:(CGRect)frame manager:(HXPhotoManager *)manager isOutside:(BOOL)isOutside delegate:(HXCustomCameraViewController *)delegate {
...
- (NSString *)getTitleLabText {
    return @"";
//    return [NSBundle hx_localizedStringForKey:@"点击拍照"];
}

- (NSString *)recTitleLabText {
    return @"";
//    return [NSBundle hx_localizedStringForKey:@"点击录制"];
}

- (void)setupPhotoType {
    self.mode = HXCustomCameraBottomViewModePhoto;
    self.titleLb.text = [self getTitleLabText];
    self.titleLb.alpha = 1;
    self.photoBtn.hidden = YES;
    self.videoBtn.hidden = YES;
    [self.playView setupPhotoPlayWithStatus:YES];
    if ([self.delegate respondsToSelector:@selector(playViewInitMode:)]) {
        [self.delegate playViewInitMode:self.mode];
    }
}
- (void)setupVideoType {
    self.mode = HXCustomCameraBottomViewModeVideo;
    self.titleLb.text = [self recTitleLabText];
    self.titleLb.alpha = 1;
    self.photoBtn.hidden = YES;
    self.videoBtn.hidden = YES;
    [self.playView setupPhotoPlayWithStatus:NO];
    if ([self.delegate respondsToSelector:@selector(playViewInitMode:)]) {
        [self.delegate playViewInitMode:self.mode];
    }
}
...
- (void)layoutSubviews {
    ...
    float theDev = [self getTheDev];
    self.photoBtn.hx_y = self.playView.hx_y - 30 - theDev;
...
- (HXFullScreenCameraPlayView *)playView {
    if (!_playView) {
        ...
        if (self.manager.configuration.customCameraType != HXPhotoCustomCameraTypeVideo) {
            [_playView setupPhotoPlayWithStatus:YES];
        }
    }
    return _playView;
}
