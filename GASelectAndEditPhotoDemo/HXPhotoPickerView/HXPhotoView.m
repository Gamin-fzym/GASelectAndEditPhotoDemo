...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (self.tempShowAddCell) {
//        // todo
//        for (HXPhotoModel *iModel in self.dataList) {
//            [self.manager beforeSelectedListAddPhotoModel:iModel];
//        }
        if (indexPath.item == self.dataList.count) {
...
- (void)goPhotoViewController {
    if (self.showSheetBlock) {
        self.showSheetBlock(1);
    }
...
- (void)photoNavigationViewController:(HXCustomNavigationController *)photoNavigationViewController didDoneAllList:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photoList videos:(NSArray<HXPhotoModel *> *)videoList original:(BOOL)original {
    ...
    [self dealWithViewController:photoNavigationViewController didDoneAllList:allList photos:photoList videos:videoList original:original];
...
- (void)dealWithViewController:(HXCustomNavigationController *)photoNavigationViewController didDoneAllList:(NSArray<HXPhotoModel *> *)allList photos:(NSArray<HXPhotoModel *> *)photoList videos:(NSArray<HXPhotoModel *> *)videoList original:(BOOL)original {
    // 上传图片后要更新allList的networkPhotoUrl
    if (self.dataList.count > 0) {
        if (allList.count > 0) {
            for (HXPhotoModel *ppModel in allList) {
                for (HXPhotoModel *llModel in self.dataList) {
                    if ([llModel.asset isEqual:ppModel.asset]) {
                        if (llModel.networkPhotoUrl) {
                            ppModel.networkPhotoUrl = llModel.networkPhotoUrl;
                        }
                        if (llModel.networkThumbURL) {
                            ppModel.networkThumbURL = llModel.networkThumbURL;
                        }
                        if (llModel.videoURL) {
                            ppModel.videoURL = llModel.videoURL;
                        }
                    }
                }
            }
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            // 处理耗时操作
            HXEditObjectViewController *editObject = [[HXEditObjectViewController alloc] initWithNibName:@"HXEditObjectViewController" bundle:nil];
            editObject.manager = self.manager;
            [editObject setupDataWithAllList:allList photos:photoList videos:videoList original:original];
            editObject.modalPresentationStyle = UIModalPresentationFullScreen;
            HXWeakSelf;
            [editObject setEditImageCompleteBlock:^(NSArray<HXPhotoModel *> * _Nonnull edit_allList, NSArray<HXPhotoModel *> * _Nonnull edit_photoList, NSArray<HXPhotoModel *> * _Nonnull edit_videoList) {
                if ([weakSelf.delegate respondsToSelector:@selector(photoListViewControllerDidDone:allList:photos:videos:original:)]) {
                    [weakSelf.delegate photoListViewControllerDidDone:self allList:edit_allList photos:edit_photoList videos:edit_videoList original:original];
                }
                if ([weakSelf.delegate respondsToSelector:@selector(photoViewCurrentSelected:photos:videos:original:)]) {
                    NSMutableArray *allModel = [NSMutableArray array];
                    NSMutableArray *photoModels = [NSMutableArray array];
                    NSMutableArray *videoModels = [NSMutableArray array];
                    NSMutableArray *tempAll = allList.mutableCopy;
                    for (HXPhotoModel *pModel in weakSelf.dataList) {
                        for (HXPhotoModel *subPModel in tempAll) {
                            if ([pModel isEqualPhotoModel:subPModel]) {
                                [tempAll removeObject:subPModel];
                                break;
                    ...
                    [weakSelf.delegate photoViewCurrentSelected:allModel.copy photos:photoModels.copy videos:videoModels.copy original:original];
                }
                [weakSelf setupDataWithAllList:edit_allList photos:edit_photoList videos:edit_videoList original:original];
            }];
            [editObject setReturnViewBlock:^(NSArray<HXPhotoModel *> * _Nonnull edit_allList, NSArray<HXPhotoModel *> * _Nonnull edit_photoList, NSArray<HXPhotoModel *> * _Nonnull edit_videoList) {
                if (weakSelf.dataList && weakSelf.dataList.count > 0) {
                    [self.manager clearCancelENDListWithList:weakSelf.dataList];
                } else {
                    [self.manager clearCancelENDList];
                }
            }];
    
            [[self topViewController] presentViewController:editObject animated:YES completion:nil];
        });
    });
}
...
- (void)setupDataWithAllList:(NSArray *)allList photos:(NSArray *)photos videos:(NSArray *)videos original:(BOOL)original {
    ...
    [self changeSelectedListModelIndex];
    [self.collectionView reloadData];
    if (!self.manager.configuration.onlyShow && [self.delegate respondsToSelector:@selector(photoView:changeComplete:photos:videos:original:)]) {
